...
Status: seem to have run out of RAM. not sure if program is entirely debugged but in areas where it appears to work, previously working lcd strings are now corrupted
...
Done:
  - in selectUserMenu() change from switch case to for loop on array of strings (5.10.2013)
...
char* userNames[] = {"Hanna","Anna","Arlin","Younghee","Minhee","Guest"}; // array of character arrays (strings)
const int namesSize = 5; // size of array above. be sure to change if adding extra names (didn't want to mess around with sizeof() )

float recordTime;
int recordTimeAddress[namesSize];  // EEPROM addresses of record times for each user. initilized in setupEE()
...
void loop() {
  ...
  switch(state) {
    ...
    case ready:
      newRecord = false; 
      recordTime = EEPROM.readFloat(recordTimeAddress[userIndex]); // get record time for currently selected user
  ...
  case done:
    ...
    if (newRecord == true) { 
      ...
      bool goodWrite = EEPROM.updateFloat(recordTimeAddress[userIndex], recordTime); 
      if (goodWrite) {
