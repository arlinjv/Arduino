...
const int autoOffPin = 18; //A4
unsigned long idleTime; // number of milliseconds of idle time. used for auto-off
...
String sketchVersion = "updated 5.26.13";
...
void setup() {
  ...
  idleTime = millis();
  pinMode(autoOffPin,OUTPUT);
  digitalWrite(autoOffPin, LOW);
  ...
  switch(state) {
    ...
    case ready:
      ...
      if (currentReading > threshold) { //still waiting or previous low value was a glitch or just a tug on the bar. keep waiting.
        ...
        if ( idleTime + 30000 < millis() ){ // if it has been 30 or more seconds since idleTime was reset to current time (millis)
          digitalWrite(autoOffPin, HIGH); // enable off pin on the power switch board
        }
