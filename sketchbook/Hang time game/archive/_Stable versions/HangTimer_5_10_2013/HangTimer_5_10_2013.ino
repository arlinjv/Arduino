...
To do:
  ...
  - in selectUserMenu() change from swich case to for loop on array of strings
...
void loop() {
  ...
  switch(state) {
    ...
    case welcome:
      ...
      if(cButton.uniquePress()){ // left and right buttons will be used iin selectUserMenu() to select user name. 
        state = ready;
  ...
  case hold:               // To do: implement auto power-off
    if(cButton.uniquePress()){ // press center button to go again
      state = welcome;
      break;
    }
    break; // end case hold:
