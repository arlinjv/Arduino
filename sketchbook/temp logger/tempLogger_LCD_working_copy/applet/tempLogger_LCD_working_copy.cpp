...
int ledPin = 13;
int pwmPin = 11;
int monitorSelectPin = 1; //when connected to ground causes LCD to continually update with temps
...
void setup() {
  ...
  pinMode(monitorSelectPin, INPUT);
...
void beginLog() {
  digitalWrite(ledPin, HIGH); // turn on on board LED to indicate logging in progress
  ...
  while ( !(Serial.available()) ) { // attach serial reset disable jumper to continue logging after disconnect
    if ( checkInterval() ) {
      //int pwmVal = map(currentAddr,0,LAST_DATA_ADDR,0,255); // works but doesn't blink. only slight difference in brightness
...
boolean menu() {
  ...
  if (digitalRead(monitorSelectPin) == HIGH) {
    getTemps();
    delay(500);
  }
