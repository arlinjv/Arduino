...
void loop() {
  ...
  switch(state){
    ...
    case in_round:
      ...
      if ((long)(millis() - endOfRound) >= 0){ //I think this approach should cover rollover situation. (see http://playground.arduino.cc/Code/TimingRollover )
        state = rest_period;
    ...
    case rest_period:
      //Serial.print("R");
      digitalWrite(runningLED, LOW); // change this to blinking (green LED). red LED should be off now
